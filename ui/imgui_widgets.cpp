...
bool ImGui::TabButton(const char* label, const char* icon, const ImVec2& size_arg, bool selected, ImGuiButtonFlags flags)
{
    ...
    it_anim->second.text_color = ImLerp(it_anim->second.text_color, selected ? ImColor(255, 255, 255, 255) : ImColor(137, 137, 137, 255), g.IO.DeltaTime * 6.f);
    it_anim->second.background_color = ImLerp(it_anim->second.background_color, selected ? ImColor(137, 137, 137, 50) : ImColor(0, 0, 0, 0), g.IO.DeltaTime * 6.f);
    it_anim->second.border_color = ImLerp(it_anim->second.border_color, selected ? ImColor(50, 50, 50, 230) : ImColor(0, 0, 0, 0), g.IO.DeltaTime * 6.f);


    // Render
    RenderNavHighlight(bb, id);
    window->DrawList->AddRectFilled(bb.Min, bb.Max, GetColorU32(it_anim->second.background_color), style.FrameRounding);
    window->DrawList->AddRect(bb.Min, bb.Max, GetColorU32(it_anim->second.border_color), style.FrameRounding);

    if (g.LogEnabled)
        LogSetNextTextDecoration("[", "]");

    ImGui::GetWindowDrawList()->AddText(icons, 18, ImVec2(bb.Min.x + 10, bb.Max.y + style.FramePadding.y - 30), GetColorU32(it_anim->second.text_color), icon);
    ImGui::GetWindowDrawList()->AddText(ImVec2(bb.Min.x + 45, bb.Max.y + style.FramePadding.y - 30), GetColorU32(it_anim->second.text_color), label);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    it_anim->second.animation = ImLerp(it_anim->second.animation, *v ? 1.0f : 0.0f, g.IO.DeltaTime * 6.f);
    it_anim->second.background_color = ImLerp(it_anim->second.background_color, *v ? ImColor(88, 87, 85, 255) : ImColor(65, 66, 67, 255), g.IO.DeltaTime * 6.f);
    it_anim->second.circle_color = ImLerp(it_anim->second.circle_color, *v ? ImColor(255, 255, 255, 255) : ImColor(95, 96, 96, 255), g.IO.DeltaTime * 6.f);

    RenderNavHighlight(total_bb, id);

    RenderFrame(frame_bb.Min, frame_bb.Max, GetColorU32(it_anim->second.background_color), false, 9.0f);

    window->DrawList->AddCircleFilled(ImVec2(frame_bb.Min.x + 8 + 14 * it_anim->second.animation, frame_bb.Min.y + 8), 5.0f, GetColorU32(it_anim->second.circle_color), 30);

    PushStyleColor(ImGuiCol_Text, ImVec4(ImColor(128,128,128)));
    if (label_size.x > 0.0f)
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, float val, ImGuiComboFlags flags)
{
    ...
    it_anim->second.text_color = ImLerp(it_anim->second.text_color, it_anim->second.opened_combo ? ImColor(255, 255, 255, 255) : hovered ? ImColor(153, 153, 153, 255) : ImColor(137, 137, 137, 255), g.IO.DeltaTime * 6.f);

    const float value_x2 = ImMax(bb.Min.x, bb.Max.x - arrow_size);
    const int vtx_idx_1 = ImGui::GetWindowDrawList()->VtxBuffer.Size;
    window->DrawList->AddRectFilled(bb.Min, ImVec2(value_x2 + 21, bb.Max.y), ImColor(100, 100, 100, 30), style.FrameRounding);
    window->DrawList->AddRect(bb.Min, ImVec2(value_x2 + 21, bb.Max.y), ImColor(100, 100, 100, 50), style.FrameRounding);

    PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.text_color));
    if (label == "WeaponConfig")
    {
        RenderTextClipped(bb.Min + ImVec2(35, 7), ImVec2(value_x2, bb.Max.y + 5), preview_value, NULL, NULL);

        PushFont(icons);
        RenderTextClipped(bb.Min + ImVec2(10, 8), ImVec2(value_x2 + 20, bb.Max.y + 5), "F", NULL, NULL);
        PopFont();
    }
    else
    {
        ...
        PushFont(icons);
        RenderTextClipped(bb.Min + ImVec2(w + 25, 8), ImVec2(value_x2 + 20, bb.Max.y + 1), "E", NULL, NULL);
        PopFont();
    ...
    PushStyleColor(ImGuiCol_WindowBg, ImVec4(ImColor(30, 31, 32)));
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    it_anim->second.text_color = ImLerp(it_anim->second.text_color, IsItemActive() ? ImColor(255, 255, 255, 255) : hovered ? ImColor(153, 153, 153, 255) : ImColor(137, 137, 137, 255), g.IO.DeltaTime * 6.f);

    window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(0, 8), frame_bb.Max - ImVec2(3, 9), ImColor(84, 84, 84), 100);
    ...
    if (grab_bb.Max.x > grab_bb.Min.x) {
        ...
        window->DrawList->AddCircleFilled(frame_bb.Min + ImVec2(it_anim->second.slow - 1, 10.f), 7, ImColor(255, 255, 255), 100);
    }
    ...
    PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.text_color));
    RenderTextClipped(frame_bb.Min - ImVec2(0, 30), frame_bb.Max - ImVec2(0, 25), value_buf, value_buf_end, NULL, ImVec2(1.0f, 1.0f));
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    it_anim->second.text_color = ImLerp(it_anim->second.text_color, selected ? ImColor(255, 255, 255, 255) : hovered ? ImColor(153, 153, 153, 255) : ImColor(137, 137, 137, 255), g.IO.DeltaTime * 6.f);
